...
public class AssetSetter { //används för att sätta in objekt/NPC i världen
    ...
    private final int obj_spikes = 3;
    ...
    public void setObject(int mapNumber) {
        ...
        while(i < objsamount) {
            ...
            else if(objPropertiesList[i].type == obj_spikes) {
                OBJ_Spikes.spike_direction = objPropertiesList[i].side;
                gp.obj[i] = new OBJ_Spikes(gp);
                gp.obj[i].worldX = gp.tileSize * objPropertiesList[i].x;
                gp.obj[i].worldY = gp.tileSize * objPropertiesList[i].y;
            }
            i++;
